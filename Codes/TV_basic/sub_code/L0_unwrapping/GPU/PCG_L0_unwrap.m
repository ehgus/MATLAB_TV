...
dx_uw = padarray(((diff(last_sol,1,1))),[1 0 0 0], 0 ) ;
dy_uw = padarray(((diff(last_sol,1,2))),[0 1 0 0], 0 ) ;
dx_w = padarray(angle(exp(1i*diff(phase_pad,1,1))),[1 0 0 0], 0 ) ;
dy_w = padarray(angle(exp(1i*diff(phase_pad,1,2))),[0 1 0 0], 0 ) ;

u=epsi_0./(abs(dx_uw-dx_w).^2+epsi_0);
v=epsi_0./(abs(dy_uw-dy_w).^2+epsi_0);

phase_diff_x =  padarray(angle(exp(1i*diff(phase_pad,1,1))),[1 0 0 0], 0 ) ;
phase_diff_y =  padarray(angle(exp(1i*diff(phase_pad,1,2))),[0 1 0 0], 0 ) ;
u_pdiff_x = u.*phase_diff_x ;
v_pdiff_y = v.*phase_diff_y ;
u_pdiff_x_diff = (diff(u_pdiff_x,1,1)) ;
v_pdiff_y_diff = (diff(v_pdiff_y,1,2)) ;
...
function Qp=compute_Qp(p,u,v,z_level)
    p_diff_x = padarray(diff(p,1,1),[1 0 0 0], 0 ) ;
    p_diff_y = padarray(diff(p,1,2),[0 1 0 0], 0 ) ;
    u_pdiff_x = u(:,:,z_level).*p_diff_x ;
    v_pdiff_y = v(:,:,z_level).*p_diff_y ;
    u_pdiff_x_diff = (diff(u_pdiff_x,1,1)) ;
    v_pdiff_y_diff = (diff(v_pdiff_y,1,2)) ;
