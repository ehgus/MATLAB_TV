	...
	int nrhs, mxArray const *prhs[])
{
	...
	for (int matt_num = 0; matt_num < Phase_sizes[2]; ++matt_num) {
		...
		for (j = xsize_actual - 1; j >= 0; j--) {
			for (i = xsize_actual - 1; i >= 0; i--) {
				if (i < xsize_actual && j < ysize_actual)
					phase[j*xsize_actual + i] = pPhase[j*xsize_actual + i + matt_num * xsize_actual*ysize_actual] * one_over_twopi;
				else phase[j*xsize_actual + i] = 0.0;
			}
		}
