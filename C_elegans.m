...
RI_bg_set=[RIdata.x0x RIdata.x20x RIdata.x30x];
%% 
for i=2:length(RI_bg_set)
    ...
    unlockTomocubeData(fullfile(sample_dir,'data3d\000000\images.dat'),fullfile(sample_dir,'data3d\000000PNG'))
    unlockTomocubeData(fullfile(sample_dir,'bgImages\calibration.dat'),fullfile(sample_dir,'bgImagesPNG'))
end
...
for i=1:length(RI_bg_set)
    ...
    %Phase=gather(unwrapp2_gpu(gpuArray(single(Phase))));
    %phaseAmp_file=fullfile(sample_dir,'phase_amplitude.mat');
    %save(phaseAmp_file,'Phase','Amplitude');
    
    %3D recontruciton taking advantage of Rytov approximation
    %rytov_params= BACKWARD_SOLVER_RYTOV.get_default_parameters(updated_params);
...
%%
function unlockTomocubeData(srcFile,dstDir)
